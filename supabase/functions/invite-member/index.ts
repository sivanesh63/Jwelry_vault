...
Deno.serve(async (req) => {
  ...
  if (!isAdmin) return json({ error: "Only an admin can invite" }, 403);
  ...
  if (familyError || !familyId) {
    return json({ error: familyError?.message ?? "No family" }, 400);
  }
  ...
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) return json({ error: "That email is not valid" }, 400);
  ...
  if (error) {
    ...
    const hint = /rate|limit/i.test(error.message)
      ...
      : "";
    return json({ error: error.message + hint }, 400);
  }

  return json({ userId: data.user?.id ?? null, email, displayName, role });
});
